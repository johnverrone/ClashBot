...
func main() {
	...
	for _, m := range war.Clan.Members {
		fmt.Printf("%s [%s] initialized with %d attacks.\n", m.Name, m.Tag, len(m.Attacks))
		prevState.AttackCounter.Count[m.Tag] = len(m.Attacks)
	}
	...
	fmt.Printf("Current attack count breakdown: (%d members)\n", len(prevState.AttackCounter.Count))
